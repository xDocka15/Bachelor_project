...
static void IRAM_ATTR mode_check(void* arg)
{
    ...
    if (gpio_get_level(GPIO_INPUT_MODE_AUTO) == 1)  //bug reseni: sepnuti civky oklame sensor v MODE_BUTTON
    {
...
{
    uint64_t bdiam_SC = 0.029 * TIMER_SCALE; // calculated 27
    uint64_t dist_to_coil_SC = 0.080 * TIMER_SCALE; // distance to the end of pulse
    delay_SC = 0; 
    delay2_SC = 0;
    if (!TIMER_DELAY_STATE && !TEMP_OVERLOAD && TIMER_READY) 
    {
        if (gpio_get_level(GPIO_INPUT_BUTTON) == 1 && gpio_get_level(GPIO_INPUT_MODE_BUTTON) == 1) 
        { 
            TIMER_STATE = true;
            TIMER_READY = false;
            //INT_TEST = true;
            ...
            timer_start(TGROUP, TNUMBER);
            LEDTEST = true;
        } else if (gpio_get_level(GPIO_INPUT_SENS) == 1 && gpio_get_level(GPIO_INPUT_MODE_AUTO) == 0) 
        { // (kulicka prisla do sensoru) reset timeru pro mereni delky zakryti senzoru
            ...
            timer_start(TGROUP, TNUMBER);
        } else if (gpio_get_level(GPIO_INPUT_SENS) == 0 && gpio_get_level(GPIO_INPUT_MODE_AUTO) == 0) 
        { // (kulicka opustila sensor)
            ...
            if (timeinsens_SC != 0)
            {
                ...
                if (delay_SC > coiluptime * TIMER_SCALE) // reseni bug vetrak spina sensor + omezeni max rychlosti
                {
                    ...
                    // if (speed_SC < 0.4 * TIMER_SCALE)
                    // {
                    //     delay2_SC = delay2_SC * 1.5 ; 
                    // } else if (speed_SC > 1 * TIMER_SCALE)
                    // {
                    //     delay2_SC = delay2_SC * 0.5;
                    // } else if (speed_SC > 1.3 * TIMER_SCALE)
                    // {
                    //     delay2_SC = delay2_SC * 0.3;
                    // }
            ...
            //bug nestiha pri male rychlosti z duvodu spomaleni mezi sensorem-civkou, reseni: ruzne mody "převody" pro ruzne rychlosti kulicky - done
...
//GPIO
    ...
    io_conf.pin_bit_mask = GPIO_INPUT_ANYEDGE_SEL;
    //set as input mode
  ...
  //  gpio_config(&io_conf);
    ...
    gpio_isr_handler_add(GPIO_INPUT_SENS, gpio_isr_handler, NULL);
...
//ADC
    ...
    while(1) {
        ...
        if (SPEED_PRINT)
        {
            ...
            double speed2 = speed2_SC;
            double delay2 = delay2_SC;
            double delay = delay_SC;
            double speed_avg = speed_avg_SC;
            if (speed > speed_max)
            {
                speed_max = speed;
            }
