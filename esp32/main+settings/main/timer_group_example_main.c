...
volatile bool timer_isr_occured = false;
...
void app_main(void)
{
    ...
    example_tg_timer_init(TIMER_GROUP_1, TIMER_0, false, 10);

    while (1) {
        if(timer_isr_occured)
        {
            printf("timer ISR occured\n");
            timer_isr_occured = false;
        }
        example_timer_event_t evt;
